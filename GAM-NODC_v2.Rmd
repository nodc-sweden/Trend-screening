...
params: 
  filename: "trenderHAV_update2025MHfix2.txt"
output: html_document
knit: (function(inputFile, encoding) {
                        rmarkdown::render(inputFile,
                        encoding=encoding, 
                        output_file="reports/GAM-NODC.html") })
---
...
data <- read_delim(file.path("data_in", filename), delim = "\t", locale = locale(encoding="UTF-8")) 
...
data <- data %>%
  ...
  # filter(count > 5) %>%
  select(value, count, Mätvariabel, Date, REG_ID, STATN) %>%
  rename(Provplats_ID = REG_ID,
         n_samples = count) %>%
  drop_na(value) %>% 
  pivot_longer(cols=c("value")) %>%
  rename(trendvariabel = Mätvariabel) 

vars <- unique(data$trendvariabel)
...
for (var in seq_along(vars)) {
  ...
  data_temp %>% 
    screeningmodeling(values=  value,
                      ...
                      Provplats_ID, trendvariabel, name, STATN) -> 
    screenmodel_out
  ...
  for(i in 1:length(unique(screenmodel_out$Provplats_ID))) {
    ...
    individual_data <- data_temp %>%
      filter(Provplats_ID==unique(screenmodel_out$Provplats_ID)[i]) %>%
      select(n_samples, Date, trendvariabel) %>%
      rename(date = Date)
    
    individual_trend_var <- individual_trend %>%
      print_individual_trend_mh(y = unique(individual_trend$name), variable = sub("_[^_]*$", "", unique(individual_data$trendvariabel))) %>%
      left_join(individual_data, by = "date") %>%
    ...
    trend <- rbind(trend, individual_trend_var)
...
trend <- trend %>%
  rename(`Provplats ID (Stationsregistret)` = Provplats_ID,
         ...
         `Trend (modelltrend)` = signif_sign,
         `Antal mätningar` = n_samples) %>%
  mutate(År = year(date),
         `Trend (modelltrend)` = gsub("0", "neutral", `Trend (modelltrend)`),
         `Trend (modelltrend)` = gsub("-1", "minskande", `Trend (modelltrend)`),
         `Trend (modelltrend)` = gsub("1", "ökande", `Trend (modelltrend)`)) %>%
  select(`Provplats ID (Stationsregistret)`, Mätvariabel, År, Årsvärde, `Trendvärde (modellvärde)`, `Trend (modelltrend)`, `Antal mätningar`)

write.table(trend,
            paste0("data_out/trends3.csv"),
            sep = ",",
            fileEncoding = "utf-8",
            row.names = FALSE)
# for(i in 1:length(unique(screenmodel_out$Provplats_ID))) {
 ...
 for(i in 1:length(unique(screenmodel_out$STATN))) {
   ...
   plot <- screenmodel_out %>%
     ...
     plot_individual_trend(y = "mean", 
                           title = 
                             #paste0(sub("_[^_]*$", "", filename), 
                             #", ", 
                           unique(screenmodel_out$STATN)[i]) 
   
   print(plot)
  
   ggsave(paste0("plots/GAM_",
                 #params$time_res,
                 "trend_",
                 #sub("_[^_]*$", "", filename),
                 #"_",
