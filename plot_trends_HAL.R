...
# Station name lookup
station_lookup <- read_tsv("data_in/trenderHAL_yearly.txt",
                           locale = locale(encoding = "UTF-8")) %>%
  distinct(REG_ID, STATN) %>%
  deframe()

trends <- read_csv("data_out/trends_HAL.csv") %>%
  rename(
...
station_order <- c(
  "N7 OST NIDINGEN", "N13 VÄRÖ", "N5 KUNGSBACKAFJORDEN",
  "N6 KUNGSBACKAFJORDEN", "N14 FALKENBERG",
  "L9 LAHOLMSBUKTEN", "ANHOLT E"
)
...
ggsave("plots/HAL_surface_trends.png", p_surface,
       width = 22, height = 12, dpi = 300, bg = "white")
message("Saved: plots/HAL_surface_trends.png")
...
ggsave("plots/HAL_bottom_O2_trends.png", p_bw,
       width = 18, height = 6, dpi = 300, bg = "white")
message("Saved: plots/HAL_bottom_O2_trends.png")
...
plot_timeseries(
  ...
  filename = "plots/HAL_surface_timeseries.png",
  width = 20, height = 12
)
...
plot_timeseries(
  ...
  filename = "plots/HAL_bottom_O2_timeseries.png",
  width = 10, height = 10
)
