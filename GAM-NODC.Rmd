...
read_tsv(file.path("data_in", filename), locale = locale(encoding="latin1")) %>%
  ...
  group_by(Provplats_ID,name) %>% # name är variabelnamn från modellen
...
write.table(individual_trend_var,
            paste0("data_out/individual_trend_",sub("_[^_]*$", "", filename),
                   "_",unique(screenmodel_out$STATN)[i],".csv"),
            sep = ",",
            fileEncoding = "utf-8",
            row.names = FALSE)
...
plot_individual_trend(y = "mean", sub("_[^_]*$", "", filename))  
 
ggsave(paste0("plots/individual_trend_", sub("_[^_]*$", "", filename),
                   "_",unique(screenmodel_out$STATN)[i],".png"), plot1)
